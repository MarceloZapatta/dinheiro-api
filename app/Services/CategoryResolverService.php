        ...
        if ($isExpense) {
            return match (true) {
                ...
                $this->containWords($description, [
                    ...
                    'drogasil',
                    'droga'
                ]) => $categories->where('expense', 1)->firstWhere('nome', 'Saúde'),
                $this->containWords($description, [
                    'viki',
                    'pastelari',
                    'casadecarne',
                ...
                $this->containWords($description, [
                    'materiais',
                    'matieli'
                ]) => $categories->where('expense', 1)->firstWhere('nome', 'Moradia'),
                $this->containWords($description, [
                    'saae',
                    'cpfl',
                    'seguro',
                    'conta vivo',
                    'contabilizei'
                ]) => $categories->where('expense', 1)->firstWhere('nome', 'Pagamentos'),
                $this->containWords($description, [
                    'mercadolivre',
                    'shopee'
                ]) => $categories->where('expense', 1)->firstWhere('nome', 'Compras'),
                $this->containWords($description, [
                    'max',
                    'cobasi',
                    'petlove'
                ]) => $categories->where('expense', 1)->firstWhere('nome', 'Pets'),
